/*
# rama - UserBot
# Copyright (C) 2021 rama-Bot Created By ZeeoneOfc
#
# This file is a part of < https://github.com/zeeoneofc/rama/ >
# PLease read the GNU Affero General Public License in
# <https://www.github.com/zeeoneofc/rama/blob/v1/LICENSE/>.
*/ 
...
module.exports = rama = async (rama, mek) => {
	try {
		...
		const q = args.join(' ')
		const botNumber = rama.user.jid
		const botNumberss = rama.user.jid + '@c.us'
		const isGroup = from.endsWith('@g.us')
		const sender = mek.key.fromMe ? rama.user.jid : isGroup ? mek.participant : mek.key.remoteJid
		const ownerNumber = [`${ownernumber}@s.whatsapp.net`] 
		const isOwner = mek.key.fromMe ? rama.user.jid : ownerNumber.includes(sender)
		const totalchat = await rama.chats.all()
		...
		const conts = mek.key.fromMe ? rama.user.jid : rama.contacts[sender] || { notify: jid.replace(/@.+/, '') }
        const pushname = mek.key.fromMe ? rama.user.name : conts.notify || conts.vname || conts.name || '-'
        ...
        const reply = (teks) => {
            rama.sendMessage(from, teks, text, {quoted:mek})
        }
        
        const reply2 = (teks) => {
rama.sendMessage(from, teks, text, {quoted: mek, contextInfo: {"forwardingScore":999,"isForwarded":true},sendEphemeral: true})

        }
        
        const mentions = (teks, memberr, id) => {
            (id == null || id == undefined || id == false) ? rama.sendMessage(from, teks.trim(), extendedText, { contextInfo: { "mentionedJid": memberr } }) : rama.sendMessage(from, teks.trim(), extendedText, { quoted: mek, contextInfo: { "mentionedJid": memberr } })
        }
			...
			const checkLimit = (sender) => {
				...
				for (let lmt of _limit) {
					if (lmt.id === sender) {
						...
						if (limitCounts <= 0) return rama.sendMessage(from,`Limit kamu sudah habis`, text,{ quoted: mek})
						rama.sendMessage(from, lang.limitcount(isPremium, limitCounts), text, { quoted : mek})
						found = true
					}
				}
					if (found === false) {
			...
			const isLimit = (sender) =>{ 
				...
				for (let i of _limit) {
					if (i.id === sender) {
						...
						if (limits >= limitawal ) {
							position = true
							rama.sendMessage(from, lang.limitend(pushname), text, {quoted: mek})
							return true
						} else {
							_limit
							position = true
        ...
        const sendStickerFromUrl = async(to, url) => {
                ...
                download(url, './stik' + names + '.png', async function () {
                    ...
                    exec(`ffmpeg -i ${filess} -vcodec libwebp -filter:v fps=fps=20 -lossless 1 -loop 0 -preset default -an -vsync 0 -s 512:512 ${asw}`, (err) => {
                        ...
                        rama.sendMessage(to, media, MessageType.sticker,{quoted:mek})
			...
			const sendMediaURL = async(to, url, text="", mids=[]) =>{
                ...
                download(url, filename, async function () {
                    ...
                    rama.sendMessage(to, media, type, { quoted: mek, mimetype: mime, caption: text,contextInfo: {"mentionedJid": mids}})
            ...
            mime = res.headers['content-type']
            if (mime.split("/")[1] === "gif") {
                return rama.sendMessage(from, await getBuffer(url), video ,{caption: caption, gifPlayback: true, mentions: men ? men : [], mimetype: 'video/mp4' ,quoted: mek})
                }
            let type = mime.split("/")[0]+"Message"
            if(mime === "application/pdf"){
                return rama.sendMessage(from, await getBuffer(url), document, {mimetype: 'application/pdf', caption: caption, mentions: men ? men : [], quoted: mek })
            }
            if(mime.split("/")[0] === "image"){
                return rama.sendMessage(from, await getBuffer(url), image ,{ caption: caption, mentions: men ? men : [], quoted: mek})
            }
            if(mime.split("/")[0] === "video"){
                return rama.sendMessage(from, await getBuffer(url), video, {caption: caption, mentions: men ? men : [], mimetype: 'video/mp4', quoted: mek})
            }
            if(mime.split("/")[0] === "audio"){
                return rama.sendMessage(from, await getBuffer(url), audio, {caption: caption, mentions: men ? men : [], mimetype: 'audio/mpeg', quoted: mek })
            }
        }
				...
				const sendButMessage = (id, text1, desc1, but = [], options = {}) => {
					const buttonMessage = {
						...
						rama.sendMessage(id, buttonMessage, MessageType.buttonsMessage, options);
					};
				const sendButLocation = async (id, text1, desc1, gam1, but = [], options = {}) => {
					them = gam1
					mediaxxaa = await rama.prepareMessage(id, them, MessageType.location, {thumbnail: them})
					...
					const buttonMessages = {
						...
						rama.sendMessage(id, buttonMessages, MessageType.buttonsMessage, options)
						}
				const sendButVideo = async(id, text1, desc1, vid1, but = [], options = {}) => {
					them = vid1
					mediaxxaa = await rama.prepareMessage(id, them, MessageType.video)
				...
				const sendButImage = async(id, text1, desc1, vid1, but = [], options = {}) => {
					them = vid1
					mediaxxaa = await rama.prepareMessage(id, them, MessageType.image, {thumbnail: Buffer.alloc(0)})
		        ...
		        await rama.sendMessage(from, `Hmm maap nih gua kick, dilarang share link di group ini`, text , {quoted: mek})
		        rama.groupRemove(from, [kic]).catch((e)=>{reply(`Bot Harus Jadi Admin`)})
		        }
			...
			const isRama = checkRegisteredUser(sender)
...
if (!isRama) return reply2(lang.noregis(pushname))
				sendButMessage(from, lang.menu(prefix, salam, pushname, readmore), '© ' + ownername, [{buttonId: '.owner', buttonText: {displayText: '𝐎𝐖𝐍𝐄𝐑'}, type: 1},{buttonId: '.infobot', buttonText:{displayText: '𝐈𝐍𝐅𝐎𝐁𝐎𝐓'}, type: 1}], {quoted: mek})
				break
case 'infobot':
if (!isRama) return reply2(lang.noregis(pushname))
			reply('.')
...
case 'owner':{
	if (!isRama) return reply2(lang.noregis(pushname))
		...
		for (let i of ownerContact.map(v => v + '@s.whatsapp.net')) {
			const vname = rama.contacts[i] != undefined ? rama.contacts[i].vname || rama.contacts[i].notify : undefined
			ini_list.push({
				"displayName": `${ownername}`,
				"vcard": `BEGIN:VCARD\nVERSION:3.0\nN:Sy;Bot;;;\nFN:${vname}\nitem1.TEL;waid=${i.split('@')[0]}:${i.split('@')[0]}\nitem1.X-ABLabel:Ponsel\nEND:VCARD`
				})
				}
				hehe = await rama.sendMessage(from, {
					"displayName": `${ini_list.length} kontak`,
					"contacts": ini_list 
					}, 'contactsArrayMessage', { quoted: mek })
					rama.sendMessage(from, `Nih Kak Contact Owner Ku, Cuma Sv Nomor Cewe Ya 🤝`, text, {quoted: hehe})
				}
			break
...
if (!isRama) return reply2(lang.noregis(pushname))
			...
			const media = await rama.downloadAndSaveMediaMessage(encmedia)
			ran = getRandom('.webp')
			await ffmpeg(`./${media}`)
			.input(media)
			...
			.on('end', function () {
				console.log('Finish')
				rama.sendMessage(from, fs.readFileSync(ran), sticker, { quoted: mek })
				fs.unlinkSync(media)
				fs.unlinkSync(ran)
				})
		...
		for(let i of result.medias){
			if(i.url.includes('mp4')){
				let link = await getBuffer(i.url)
                    rama.sendMessage(from,link,video,{thumbnail: Buffer.alloc(0), quoted: mek,caption: `Instagram •  ${i.type}`})
                } else {
                    let link = await getBuffer(i.url)
                    rama.sendMessage(from,link,image,{thumbnail: Buffer.alloc(0), quoted: mek,caption: `Instagram • ${i.type}`})                  
                }
            }
...
if (!isRama) return reply2(lang.noregis(pushname))
			...
			.then(result => {
				...
				.then(async (a) => {
					...
					rama.sendMessage(from,noweem , MessageType.video, {mimetype: 'video/mp4',quoted: mek})
					})
...
if (!isRama) return reply2(lang.noregis(pushname))
			...
			.then(result => {
				...
				.then(async (a) => {
					...
					rama.sendMessage(from,weem , MessageType.video, {mimetype: 'video/mp4',quoted: mek})
					})
				}).catch((err) => reply(`Link tidak valid`))
...
if (!isRama) return reply2(lang.noregis(pushname))
			...
			.then(result => {
				...
				.then(async (a) => {
					...
					rama.sendMessage(from,audnha , MessageType.document, {mimetype: 'audio/mp4',filename: `Tiktok Music.mp3`,quoted: mek})
					})
				}).catch((err) => reply(`Link tidak valid`))
...
case 'spotify':{
    ...
    await rama.sendMessage(from, thumbnail, image, { quoted: lol, caption: ini_txt })
    get_audio = await getBuffer(get_result.link)
    await rama.sendMessage(from, get_audio, audio, { mimetype: 'audio/mpeg', filename: `${get_result.title}.mp3`, quoted: mek})
    }
    break
case 'spotifysearch':{
	if (!isRama) return reply2(lang.noregis(pushname))
...
case 'nhentai':{
 ...
 case 'nhentaipdf':{
    ...
    await rama.sendMessage(from, ini_buffer, document, { quoted: mek, mimetype: Mimetype.pdf, filename: `${henid}.pdf` })
    }
    break
case 'nhentaisearch':{
	if (!isRama) return reply2(lang.noregis(pushname))
...
if (!isRama) return reply2(lang.noregis(pushname))
    if (args.length == 0) return reply(`Example: ${prefix + command} rama`)
    ini_txt = args.join(" ")
    var po = await getBuffer(`https://api.lolhuman.xyz/api/ephoto1/${command}?apikey=${lolkey}&text=${ini_txt}`)
    rama.sendMessage(from, po, image, { quoted: mek, caption: "Selesai" })
    }
    break
//maker textpro
...
if (!isRama) return reply2(lang.noregis(pushname))
    ...
    rama.sendMessage(from, gambar, image, { quoted: mek, caption: "Sukses" })
    }
    break
//islami
case 'listsurah':{
...
case 'alquranaudio':{
    ...
    await rama.sendMessage(from, ini_buffer, audio, { quoted: mek, mimetype: 'audio/mpeg' })
    }
    break
case 'jadwalsholat':{
	if (!isRama) return reply2(lang.noregis(pushname))
...
case 'daftar': case 'verify': case 'verif':
			if (isRama) return  reply(lang.regis())
			try {
					ppregis = await rama.getProfilePicture(sender)
				} catch {
...
if (!isRama) return reply2(lang.noregis(pushname))
			...
			if (!isBotGroupAdmins) return reply(lang.adminB())
				if (mek.message.extendedTextMessage === undefined || mek.message.extendedTextMessage === null) return reply('Tag target yang ingin di tendang!')
					...
					if (mentioned.length > 1) {
						...
						rama.groupRemove(from, mentioned)
					} else {
			...
			if (!isBotGroupAdmins) return reply(lang.adminB())
				if (mek.message.extendedTextMessage === undefined || mek.message.extendedTextMessage === null) return reply('Tag target yang ingin di tendang!')
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					setTimeout( () => {
					rama.sendMessage(from, 'Yok Sama" Al-fatihah', text)
					}, 8000)
					setTimeout( () => {
					reply('sukses min:D')
					}, 7000)
					...
					setTimeout( () => {
					rama.sendMessage(from, `Bismilah Kick @${mentioned[0].split('@')[0]}`, text) // ur cods
					}, 5000)
					setTimeout( () => {
					rama.sendMessage(from, 'Asikkk Dapet Makanan nihh:D', text)
					}, 2500)
					setTimeout( () => {
			...
			if (!isBotGroupAdmins) return reply(lang.adminB())
				if (mek.message.extendedTextMessage === undefined || mek.message.extendedTextMessage === null) return reply('Tag target yang ingin di jadi admin!')
					...
					if (mentioned.length > 1) {
						...
						mentions(teks, mentioned, true)
						rama.groupMakeAdmin(from, mentioned)
					} else {
...
case 'memegenerator': case 'memegen':{
									...
									try {
									   ...
									   var mediiia = await rama.downloadMediaMessage(enmedia)
										...
										rama.sendMessage(from, resu, image, {caption:'.stikerin bang', thumbnail: Buffer.alloc(0), quoted: mek})
						...
						if (args.length < 1) return reply(`Kirim perintah *${prefix + command}* ramabot`)
									if (q.includes('|')) return reply(`Kirim perintah *${prefix + command}* ramabot`)
									try {
										if (!isQuotedImage) return reply(`Reply Gambar!`)
										reply(lang.wait())
										var teks2 = args.join(' ')
										var enmedia = isQuotedImage ? JSON.parse(JSON.stringify(mek).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo : mek
										var mediia = await rama.downloadMediaMessage(enmedia)
...
case 'leave':
			...
			setTimeout( () => {
			rama.groupLeave(from) 
			}, 2000)
			setTimeout( () => {
			rama.sendMessage(from, 'Sayonara👋', text)
			}, 0)
			break
case 'hidetag':
if (!isRama) return reply2(lang.noregis(pushname))
			...
			if (!isGroupAdmins) return reply(lang.admin(groupName))
			var value = q
			var group = await rama.groupMetadata(from)
			...
			rama.sendMessage(from, options, text)
			break
case 'linkgrup':case 'linkgroup': case 'linkgc':
if (!isRama) return reply2(lang.noregis(pushname))
			if (!isGroup) return reply(lang.group())
			linkgc = await rama.groupInviteCode(from)
			yeh = `https://chat.whatsapp.com/${linkgc}\n\nlink Group *${groupName}*`
			rama.sendMessage(from, yeh, text, { quoted: mek })
			break  
case 'tagall':
if (!isRama) return reply2(lang.noregis(pushname))
			if (!isGroup) return reply(lang.group())
...
if (!isRama) return reply2(lang.noregis(pushname))
			...
			if (!isBotGroupAdmins) return reply(lang.adminB())
					...
					rama.sendMessage(from, `Sukses Mengubah Nama Grup Menjadi ${q}`, text, { quoted: mek })
			break          
...
if (!isRama) return reply2(lang.noregis(pushname))
			...
			if (!isBotGroupAdmins) return reply(lang.adminB())
					...
					rama.sendMessage(from, `Sukses Mengubah Desk Grup Menjadi ${q}`, text, { quoted: mek })
			break   
case 'kick':
if (!isRama) return reply2(lang.noregis(pushname))
			if (!isGroup) return reply(lang.group())
			...
			await rama.groupRemove(from, [kickya])
...
case 'bc': case 'broadcast':
			...
			var bcnya = await rama.chats.all()
			...
			var bcnya3 = await rama.downloadMediaMessage(bcnya2)
					for (let _ of bcnya) {
						rama.sendMessage(_.jid, bcnya3, image, { caption: `*----「  BROADCAST 」----*\n\n${q}` })
						}
...
case 'nightcore':{
	 if (!isQuotedAudio) return reply('Reply audio nya om')
					...
					media = await rama.downloadAndSaveMediaMessage(encmedia)
					ran = getRandom('.mp3')
					exec(`ffmpeg -i ${media} -filter:a atempo=1.06,asetrate=44100*1.25 ${ran}`, (err, stderr, stdout) => {
						fs.unlinkSync(media)
						if (err) return reply('Error!')
						hah = fs.readFileSync(ran)
						rama.sendMessage(from, hah, audio, {mimetype: 'audio/mp4', ptt:true, quoted: mek,duration:99999999999999999999999})
						fs.unlinkSync(ran)
					   })}
				  break      
case 'bass': {
...
case 'slowmo': case 'slow':{
								try {
										...
										exec(`ffmpeg -i ${media} -filter:a "atempo=0.7,asetrate=44100" ${ran}`, (err, stderr, stdout) => {
											...
											rama.sendMessage(from, uhh, audio, {mimetype: 'audio/mp4', ptt:true, quoted: mek})
											fs.unlinkSync(ran)
											})
											} catch (e) {
												reply('Error!')
...
case 'robot':{
									...
									exec(`ffmpeg -i ${media} -filter_complex "afftfilt=real='hypot(re,im)*sin(0)':imag='hypot(re,im)*cos(0)':win_size=512:overlap=0.75" ${ran}`, (err, stderr, stdout) => {
										...
										if (err) return reply('Error!')
										hah = fs.readFileSync(ran)
										rama.sendMessage(from, hah, audio, {mimetype: 'audio/mp4', ptt: true, quoted: mek})
										fs.unlinkSync(ran)
										})
										}
									break
...
case 'tupai':{
									try {
										...
										exec(`ffmpeg -i ${media} -filter:a "atempo=0.5,asetrate=65100" ${ran}`, (err, stderr, stdout) => {
											...
											if (err) return reply('Error!')
											hah = fs.readFileSync(ran)
											rama.sendMessage(from, hah, audio, {mimetype: 'audio/mp4', ptt:true, quoted: mek,duration: 999099})
											fs.unlinkSync(ran)
											})
											 } catch (e) {	
												reply(mess.error)
...
case 'toimg':{
		if (!isQuotedSticker) return reply('Reply stc nya!')
					...
					mediaa = await rama.downloadAndSaveMediaMessage(encmediaa)
					ran = getRandom('.png')
					exec(`ffmpeg -i ${mediaa} ${ran}`, (err) => {
					fs.unlinkSync(mediaa)
					if (err) return reply('Eror')
					buffer = fs.readFileSync(ran)
					rama.sendMessage(from, buffer, image, {quoted: mek, thumbnail:Buffer.alloc(0), caption: 'Done'})
					fs.unlinkSync(ran)
					})
					}
					break   
case 'nuliskiri':{
									...
									.on('exit', () => {
										rama.sendMessage(from, fs.readFileSync('./database/media/nulis/images/buku/setelahkiri.jpg'), image, {thumbnail:Buffer.alloc(0),quoted: mek, caption: `Hati-hati ketahuan!`})
						...
						case 'nuliskanan':{
									...
									.on('error', () => reply(mess.error))
									.on('exit', () => {
										rama.sendMessage(from, fs.readFileSync('./database/media/nulis/images/buku/setelahkanan.jpg'), image, {thumbnail:Buffer.alloc(0),quoted: mek, caption: `Hati-hati ketahuan!`})
						...
						case 'foliokiri':{
									...
									.on('error', () => reply(mess.error))
									.on('exit', () => {
										rama.sendMessage(from, fs.readFileSync('./database/media/nulis/images/folio/setelahkiri.jpg'), image, {thumbnail:Buffer.alloc(0),quoted: mek, caption: `Hati-hati ketahuan!`})
						...
						case 'foliokanan':{
									...
									.on('error', () => reply(mess.error))
									.on('exit', () => {
										rama.sendMessage(from, fs.readFileSync('./database/media/nulis/images/folio/setelahkanan.jpg'), image, {thumbnail:Buffer.alloc(0),quoted: mek, caption: `Hati-hati ketahuan!`})
...
try{
                zee.Facebook(`${q}`).then(async data => {
                    ...
                    rama.sendMessage(from, ppfb, video, {mimetype:'video/mp4', quoted: mek, caption: txt})
             })} catch {
   ...
   case 'soundcloud':
	if (!isRama) return reply2(lang.noregis(pushname))
				...
				zee.SoundCloud(`${q}`).then(async (data) => {
                    ...
                    rama.sendMessage(from , await getBuffer(data.medias[0].url), audio,{ quoted: mek, mimetype: 'audio/mp4' })
				})
			break
	case 'telesticker': case 'tstiker': {
		if (!isRama) return reply2(lang.noregis(pushname))
			...
			rama.sendMessage(from, await getBuffer(telestc[i].url), sticker, {mimetype:'image/webp',quoted: mek})
			}
		}
		break
case 'tebakgambar':
...
fs.writeFileSync("./database/game/tebakgambar.json", JSON.stringify(tebakgambar))
console.log(jawaban)
rama.sendMessage(from, tebakya, image, {quoted: mek, caption: "\n\nTimeout : 120.00 seconds" })
await sleep(120000)
if (tebakgambar.hasOwnProperty(sender.split('@')[0])) {
rama.sendMessage(from, "Waktu permainan habis" + '\n\n' +"*Jawaban :*"  + '\n' + '_'+ jawaban +'_', text, {quoted: mek}) // ur cods
delete tebakgambar[sender.split('@')[0]]
fs.writeFileSync("./database/game/tebakgambar.json", JSON.stringify(tebakgambar))
                    }
                    })
...
reply(lang.wait())
	emoji.get(`${qes}`).then(async emojii => {
					...
					console.log(teks)
					//rama.sendMessage(from, await getBuffer(teks), sticker, {mimetype:'image/webp',quoted: mek})
...
case 'emojimix':
	...
	if (args.length < 1) return reply('emoji??')
				reply(lang.wait())
					...
					buff = `https://api.lolhuman.xyz/api/emojimix/${args[0]}?apikey=${lolkey}`
					voss = await fetch(buff)
					ftype = require('file-type')
					vuss = await ftype.fromStream(voss.body)
					if (vuss !== undefined) {
					costum(await getBuffer(buff), image, RAMAGans, `R-BOT`)
					} else {
...
if (!isRama) return reply2(lang.noregis(pushname))
			...
			zee.Youtube(mulaikah).then(async (data) => {
             ...
             case 'trigger':
					...
					owgi = await  rama.downloadAndSaveMediaMessage(ger)
					anu = await imgbb("4bbcd355f96fad75a74cbb89f7381afc", owgi)
					...
					rama.sendMessage(from, fs.readFileSync(rano), sticker, {quoted: mek})
					fs.unlinkSync(rano)
					})
					} else {
					reply('Gunakan foto!')
...
try {
	if (!(mek.key.fromMe && isOwner)) return reply(lang.owner(botname))
return rama.sendMessage(from, JSON.stringify(eval(budy.slice(2)),null,'\t'),text, {quoted: mek})
} catch(err) {
e = String(err)
reply(e)
}
...
qur = budy.slice(2)
exec(qur, (err, stdout) => {
if (err) return reply(`ramaBot :~ ${err}`)
if (stdout) {
reply(stdout)
}
})
